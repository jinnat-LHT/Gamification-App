...
| 2026-08-15 | Assignments are not numerically scored; Facilitator tracks review status and feedback, while completion XP remains separate. Post-test uses the same Batch question set but randomizes question and option order per Learner. | Approved | Keep practical work qualitative and preserve comparable knowledge measurement while reducing answer-copying. | Store presentation order and content Version with each result; Assignment status/feedback is separate from XP. |
