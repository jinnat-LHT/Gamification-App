...
function normaliseCriteria(value:any){
  ...
  return value.map((item:any,index:number)=>{
    ...
    const labels:any={};
    for(let n=1;n<=5;n++){labels[String(n)]=String(item?.scale_labels?.[String(n)]??"").trim();if(!labels[String(n)])throw new Error("กรุณาระบุคำอธิบายระดับ "+n+" ของข้อที่ "+(index+1));}
    return {key:String(item?.key??defaultCriteria[index].key),title,description,scale_labels:labels};
  });
}
...
Deno.serve(async(req)=>{
  ...
  if(action==="criteria-get"||action==="criteria-save"){
    ...
    if(action==="criteria-get"){const source=(configs.data??[]).find((row:any)=>Array.isArray(row.config_json?.criteria)&&row.config_json.criteria.length===5);return out({criteria:source?.config_json?.criteria??defaultCriteria,items:configs.data??[]});}
    try{
      const criteria=normaliseCriteria(payload.criteria);if((configs.data??[]).length===0)return out({error:"ยังไม่มี Self-assessment ใน Batch นี้"},404);
      for(const config of configs.data??[]){const update=await db.from("batch_activity_configs").update({config_json:{...(config.config_json??{}),criteria}}).eq("id",config.id);if(update.error)throw new Error("Could not save Self-assessment configuration");}
      await db.from("audit_events").insert({actor_user_id:user.id,batch_id:batchId,event_type:"SELF_ASSESSMENT_CRITERIA_UPDATED",target_type:"batch",target_id:batchId,after_json:{criteria}});
      return out({criteria});
  ...
  if(action==="start")return out({type,criteria,submitted:existing.data?.status==="SUBMITTED",submitted_at:existing.data?.submitted_at??null});
