(() => {
  ...
  async function invoke(action, rows) {
    ...
    const { data, error } = await client.functions.invoke("import-quiz-bank", { body: { action, batch_code: batchCode, rows } });
  ...
  function open() {
    ...
    modal.innerHTML = `<div class="max-w-2xl mx-auto my-8 glass-card p-6 rounded-2xl"><div class="flex justify-between"><div><h2 class="text-xl font-bold">นำเข้าคำถาม Quiz Bank</h2><p class="text-sm text-slate-400">Batch: ${batchCode} · รองรับ CSV</p></div><button id="closeQuizImport">✕</button></div><a class="mt-3 inline-block text-cyan-300 underline text-sm" href="./templates/quiz-bank-import-template.csv" download>ดาวน์โหลด template</a><input id="quizImportFile" type="file" accept=".csv,text/csv" class="mt-5 block w-full text-sm"/><div id="quizImportResult" class="mt-5 text-sm"></div></div>`;
