(() => {
  ...
  async function load(){
    ...
    try{
      ...
      body.innerHTML=(items.map(x=>`<div class="mb-3 rounded-xl border border-slate-800 bg-slate-900/70 p-4"><div class="flex flex-wrap items-center justify-between gap-3"><div><div class="font-semibold">${escapeHtml(x.config_json?.title||label[x.activity_type]||x.activity_key)}</div><div class="text-xs text-slate-400">${escapeHtml(x.activity_key)}</div></div><div class="flex items-center gap-2"><label class="text-xs"><input data-enabled="${x.id}" type="checkbox" ${x.enabled?"checked":""}> เปิดใช้</label><select data-gate="${x.id}" class="rounded bg-slate-800 p-2 text-xs"><option value="OPEN" ${x.gate_state==="OPEN"?"selected":""}>เปิดรับ</option><option value="LOCKED" ${x.gate_state==="LOCKED"?"selected":""}>ล็อก</option></select></div></div>${x.activity_type==="ASSIGNMENT"?`<div class="mt-4 grid gap-2"><input data-title="${x.id}" value="${escapeHtml(x.config_json?.title||x.activity_key)}" class="rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm" placeholder="ชื่อโจทย์"><textarea data-instructions="${x.id}" rows="3" class="rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm" placeholder="คำชี้แจง">${escapeHtml(x.config_json?.instructions||"")}</textarea></div>`:""}</div>`).join("")||"<p class='text-slate-400'>ไม่มีการตั้งค่ากิจกรรม</p>")+ `<div class="sticky bottom-0 mt-5 border-t border-slate-800 bg-slate-950/95 pt-4"><p id="activitySaveMessage" class="mb-2 text-sm"></p><button id="saveAllActivities" class="rounded-xl bg-cyan-500 px-5 py-3 font-bold text-slate-950">บันทึกการเปลี่ยนแปลงทั้งหมด</button></div>`;
      document.getElementById("saveAllActivities").onclick=async()=>{
        ...
        try{
          for(const item of items){
            ...
            if(item.activity_type==="ASSIGNMENT") await api({action:"update_details",batch_id:batchId,activity_config_id:item.id,title:body.querySelector(`[data-title='${item.id}']`).value.trim(),instructions:body.querySelector(`[data-instructions='${item.id}']`).value.trim()});
          }
