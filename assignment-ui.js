(() => {
  ...
  async function open(){
    ...
    let statusByConfig=new Map();
    try { const progress=await api({action:"status"}); statusByConfig=new Map((progress.items||[]).map(x=>[x.activity_config_id,x])); } catch (_) {}
    ...
    body.innerHTML=items.map((x,i)=>{const progress=statusByConfig.get(x.id);const review=progress?.feedback;const statusText=review?.status==="NEEDS_REVISION"?"ต้องแก้ไข":progress?.status==="REVIEWED"?"ตรวจแล้ว":progress?.status==="SUBMITTED"?"รอตรวจ":"ยังไม่ส่ง";const tone=review?.status==="NEEDS_REVISION"?"text-amber-300":progress?.status==="REVIEWED"?"text-emerald-300":"text-slate-400";return `<button data-assignment="${x.id}" class="mb-3 block w-full rounded-xl border border-slate-700 bg-slate-900/80 p-4 text-left hover:border-cyan-400"><div class="flex justify-between gap-3"><div class="font-semibold">${escapeHtml(x.config_json?.title||x.activity_key||`Assignment ${i+1}`)}</div><span class="text-xs ${tone}">${statusText}</span></div><div class="mt-1 text-xs text-slate-400">${x.due_at?"กำหนดส่ง "+new Date(x.due_at).toLocaleString("th-TH"):"เปิดส่งงาน"}</div>${review?.feedback_text?`<div class="mt-3 rounded-lg bg-emerald-500/10 p-3 text-sm text-emerald-200">Feedback: ${escapeHtml(review.feedback_text)}</div>`:""}</button>`;}).join("");
